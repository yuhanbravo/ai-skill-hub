...
- No landed project-side runtime pack yet; `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, and `.github/agents/*.agent.md` remain future targets rather than active repository assets.
- No final decision yet on the thinnest project-side runtime-pack primary entrypoint; the repository has not chosen between `AGENTS.md` and `.github/copilot-instructions.md` as the first project-side top-level doorway.
- No enforced backlink rule yet from that future primary entrypoint back to canonical guidance; the mapping has been documented, but the mandatory project-side pointer contract is still undecided.
...
The objective is still not to add more surfaces or a controller framework, but to make the existing layers more trustworthy, more enforceable, and easier for future maintainers or AI agents to operate without ambiguity. After the completed Phase 1 canonical workflow asset, the next directional question is narrower and should remain decision-oriented: determine what the thinnest project-side runtime-pack primary entrypoint should be, and only then decide whether more concrete project-side template design is justified.

In other words, the system should treat the just-landed `workflow-bootstrap` skill as a canonical workflow-shell clarification, not as the start of a project-side rollout. The next phase should first resolve the boundary between `AGENTS.md` and `.github/copilot-instructions.md` as the possible project-side top-level entrypoint, then clarify how that future entrypoint must point back to canonical guidance, while preserving the current wrapper-only invocation model, read-only governance posture, active-source documentation ownership, and the explicit choice not to implement the full runtime-pack file family yet.
