...
The latest completed Phase 1 addition is a new canonical skill, `workflow-bootstrap`, which gives the repository its first minimal canonical expression of the default `Copilot 主控 / Codex 施工` chain. This new skill is intentionally narrow: it defines the workflow shell, the default `planner / implementer / reviewer` role split, and the mapping between canonical guidance and a future project-side runtime pack, but it does not implement the project-side runtime pack itself. `chatgpt-handoff-pilot` remains the protocol layer for `task package`, `bounded execution`, and `execution report`, so the new workflow-shell capability clarifies coordination without replacing the existing handoff contract.

This means the repository now has a clearer canonical workflow story than it did in the previous handoff: the hub can express how planning, bounded execution, and review should line up by default, while still keeping `skills/` as the only canonical layer, `.agents/skills/` and `.github/skills/` as discoverability/adapter surfaces, and the bridge layer as continuity/exchange rather than ownership. At the same time, the system has not yet crossed into project-side runtime-pack delivery: `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, and `.github/agents/*.agent.md` are still future targets only, documented in canonical mapping assets rather than landed as active repository surfaces. Validation for the new hub-side workflow skill was actually rerun and passed, and the choice not to refresh `skills_index.json` was deliberate because the current generation path would have expanded the diff surface beyond the bounded Phase 1 whitelist.

The newly landed Phase 2 follow-up remains canonical-only and section-bounded: `workflow-bootstrap` now includes three thin-entry draft assets that document the future project-side entry model (`AGENTS.md` as the thinnest primary entry draft, `.github/copilot-instructions.md` as a Copilot-specific thin adapter draft, and canonical backreference rules draft). This improves entrypoint clarity without changing ownership boundaries: these drafts are design assets in `skills/workflow-bootstrap/`, not active project-side runtime-pack files, and there is still no rollout/distribution activation of a real runtime-pack file family in this repository.
...
- No real project-side runtime-pack files are implemented yet; `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, and `.github/agents/*.agent.md` remain future targets, even though thin-entry canonical drafts now exist in `skills/workflow-bootstrap/`.
- No project-side enforced backlink contract yet in runtime execution surfaces; backlink and no-second-rulebook constraints are now drafted canonically but not yet activated via a shipped project-side runtime pack.
...
The objective is still not to add more surfaces or a controller framework, but to make the existing layers more trustworthy, more enforceable, and easier for future maintainers or AI agents to operate without ambiguity. After the completed Phase 1 canonical workflow asset and Phase 2 thin-entry canonical drafts, the next directional question remains bounded: decide whether to progress from canonical drafts into a tightly scoped project-side template proposal, while preserving hard boundaries against immediate runtime-pack rollout.

In other words, the system should continue treating `workflow-bootstrap` as canonical workflow-shell guidance plus thin-entry draft assets, not as the start of a project-side rollout. The next phase should preserve the current wrapper-only invocation model, read-only governance posture, active-source documentation ownership, and the explicit hard boundary that the full project-side runtime-pack file family is still not implemented in this repository.
