...
The completed `workflow-bootstrap` line now spans three bounded steps without changing system phase: Phase 1 established the canonical workflow shell for the default `Copilot 主控 / Codex 施工` chain; Phase 2 added thin-entry canonical drafts (`AGENTS.md` master-entry draft, `.github/copilot-instructions.md` thin-adapter draft, and canonical backreference rules draft); and Phase 3A has now pushed that work into canonical template sketch assets for a future project-side runtime pack. `chatgpt-handoff-pilot` remains the protocol layer for `task package`, `bounded execution`, and `execution report`, so `workflow-bootstrap` still clarifies workflow shape without replacing the repository's handoff contract.

The newly completed Phase 3A addition makes the project-side template problem materially more concrete while staying canonical-only. `workflow-bootstrap` now includes a runtime-pack template sketch that classifies the future v1 file family as `required / optional / not recommended`, an `AGENTS.md` template sketch that separates fixed fields from project-filled fields, and a `.github/copilot-instructions.md` template sketch that tightens the thin-adapter boundary. The same sketch set also captures the current preferred path-backreference expression for canonical guidance in consumer repos and brings anti-second-rulebook constraints into the template layer instead of leaving them only at the draft-rule level.

This improves template readiness without crossing into runtime-pack delivery. The new Phase 3A assets live only inside `skills/workflow-bootstrap/` as canonical inputs for future implementation; they are not real project-side runtime files, they do not activate rollout/distribution/adoption, and they do not start the single consumer repo pilot implementation yet. The repository therefore remains in the same controlled-system phase with the same layer boundaries: `skills/` stays canonical, `.agents/skills/` and `.github/skills/` stay derivative discoverability surfaces, the bridge layer stays continuity/exchange rather than ownership, and the actual project-side file family (`AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, `.github/agents/*.agent.md`) is still not landed here.
...
- No real project-side runtime-pack files are implemented yet; `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md`, and `.github/agents/*.agent.md` remain future targets, even though Phase 2 thin-entry drafts and Phase 3A canonical template sketches now exist in `skills/workflow-bootstrap/`.
- No project-side enforced backlink contract yet in runtime execution surfaces; backlink, path-backreference, and no-second-rulebook constraints are now sketched canonically but not yet activated via a shipped project-side runtime pack.
- No single consumer repo pilot implementation has started yet; concrete placeholder resolution, local entrypoint combination, and project-specific template fitting remain future implementation inputs rather than completed system capabilities.
...
- No distribution / rollout / adoption step has started for the future project-side runtime pack; Phase 3A completed only the canonical template sketch layer and intentionally stopped before consumer-repo writes.
...
The next phase should still move the system from `controlled local governance` toward `controlled enforcement`, but the workflow-bootstrap track now has a narrower immediate direction: move from completed canonical template sketches into a tightly bounded single consumer repo pilot implementation task package, without skipping straight to distribution or multi-repo rollout.

Directionally, this means choosing one real consumer repo, resolving only that repo's concrete canonical path placeholders and entrypoint combination, and validating whether the current `AGENTS.md`-first / `.github/copilot-instructions.md`-thin-adapter / forced-canonical-backreference / no-second-rulebook model survives contact with an actual project layout. The purpose is still to reduce ambiguity without adding a controller framework or widening the layer surface prematurely.

In other words, the system should now treat `workflow-bootstrap` as canonical workflow-shell guidance plus Phase 2 thin-entry drafts plus Phase 3A template sketches, while still preserving the hard boundary that future project-side runtime pack surfaces are not yet implemented here. The next directional step is a single-repo pilot input package, not direct rollout, not distribution activation, and not a broader template proliferation across `.github/instructions/*.instructions.md` or `.github/agents/*.agent.md`.
