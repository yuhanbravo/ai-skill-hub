...
- Status: `stable`
- Canonical layer 仍然是唯一事实源；当前 working tree 已具备 `11` 个正式 skill，本轮新增的 `workflow-bootstrap` 补齐了“`Copilot 主控 / Codex 施工` 默认链路”的最小 canonical 表达，但它仍只停留在 workflow shell 层，而不是 project-side runtime pack 实现。
- `workflow-bootstrap` 当前只承载三类最小能力：`planner / implementer / reviewer` role split、canonical guidance 到 future runtime pack 的映射说明、以及 bounded execution 协同方式；它没有替代 `chatgpt-handoff-pilot`，后者仍然是 handoff / bounded execution / execution report 的协议层。
...
- Main direction: 继续在不打破层边界的前提下，把本地可重复流程推进到更可信的跨层治理与更低漂移维护；下一阶段应先收敛 project-side 最薄主入口的判断，而不是直接铺开 runtime pack 模板集合。
...
- 先收敛 project-side runtime pack 的最薄主入口判断，明确它应该由 `AGENTS.md` 还是 `.github/copilot-instructions.md` 承担仓库级首入口角色。
- 在主入口判断稳定之前，不直接铺开 `.github/instructions/*.instructions.md` 或 `.github/agents/*.agent.md` 模板集合，避免把 canonical workflow mapping 过早升级为 project-side 模板面。
