...
def main() -> int:
    ...
    command = python_command(
        str(REPO_ROOT / ".codex" / "skills" / "documentation-governance" / "scripts" / "check_documentation_governance.py"),
        "--root",
        str(temp_repo),
        "--config",
        str(base_dir / "governance.json"),
        "--dry-run",
        "--json",
    )
    ...
    if result.returncode != 0:
        ...
        diffs.extend(compare_json(payload, expected))
    passed, message = format_result("documentation-governance", diffs, result.stderr)
    print(message)
