...
- First executable migration task package prompt: [prompts/first_executable_migration_task_package_prompt.md](prompts/first_executable_migration_task_package_prompt.md)
- Invocation examples: [examples/invocation_examples.md](examples/invocation_examples.md)
- First executable migration task package template: [templates/FIRST_EXECUTABLE_MIGRATION_TASK_PACKAGE_TEMPLATE.md](templates/FIRST_EXECUTABLE_MIGRATION_TASK_PACKAGE_TEMPLATE.md)
...
首个 bounded migration task package 的生成物结构已拆分到 [templates/FIRST_EXECUTABLE_MIGRATION_TASK_PACKAGE_TEMPLATE.md](templates/FIRST_EXECUTABLE_MIGRATION_TASK_PACKAGE_TEMPLATE.md)。

使用边界：

- 该 template 只定义下一轮 bounded execution 的生成物结构，不代表本 skill 自动执行迁移。
- 生成调用入口见 [prompts/first_executable_migration_task_package_prompt.md](prompts/first_executable_migration_task_package_prompt.md)。
- 输出必须实例化到一个最小可执行步骤，并保留 assessment 与 execution 分离。
