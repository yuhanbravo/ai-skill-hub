...
## 10) Specialized Orchestration Snippets

Use these focused canonical assets for medium-complexity orchestration playbooks that remain owned by `workflow-bootstrap` but are too heavy for the minimal role-chain snippet library:

- [Post-Dev Dual Refresh](orchestration/post_dev_dual_refresh.md) — post-development status refresh, governed transfer block, handoff closure, merged receipt, validation provenance, branch/head provenance, and final reviewer checklist.
- [GitHub PR Bootstrap](orchestration/github_pr_bootstrap.md) — local-to-PR bootstrap flow covering branch publication and review-request preparation.

These files preserve the specialized orchestration semantics under `skills/workflow-bootstrap/` without expanding this entry file into a second rulebook.
